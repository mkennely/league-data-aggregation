...
import fs from 'fs';
import charsetParser from 'charset-parser';
...
function Crawler (options) {
  ...
  if (['onDrain', 'preRequest','cache'].some(key => key in options)) {
    throw new Error(
      'Support for \'onDrain\', \'preRequest\', \'cache\' has been removed!'
      + 'For more details, see https://github.com/bda-research/node-crawler'
    );
  }
...
Crawler.prototype.init = function init (options) {
  ...
  self.limiters = new Bottleneck.Cluster(
    self.options.maxConnections,
    self.options.rateLimit,
    self.options.priorityRange,
    self.options.priority,
    self.options.homogeneous
   );
...
Crawler.prototype._inject = function _inject (response, options, callback) {
  ...
  if (options.jQuery === 'whacko') {
    ...
    let jsdom = options.jQuery.jsdom;
    let scriptLocation = path.resolve(__dirname, '../vendor/jquery-2.1.1.min.js');
    ...
    readJqueryUrl(scriptLocation, function(err, jquery) {
      try {
        ...
        options.callback(e, {options}, options.release, self);
      }
    });
  } else {
    // Jquery is set to false or not set
    callback(null, response, options);
  }
};
...
Crawler.prototype.queue = function queue (options) {
  ...
  for (let i = 0; i < options.length; ++i) {
    if (_.isNull(options[i]) || _.isUndefined(options[i])
...
self._deleteEntity(options);
  let ropts = JSON.parse(JSON.stringify(options));
  ...
  let requestArgs = ['uri','url','qs','method','headers','body','form','json','multipart','followRedirect',
...
Crawler.prototype._onContent = function _onContent (error, options, response) {
  ...
  if (error) {
    ...
    if (options.retries) {
      ...
      options.callback(error,{options:options},options.release,self);
    }
  ...
  try {
    ...
    log('error', e);
    return options.callback(e, {options:options}, options.release, self);
  }

  response.options = options;

  if (options.method === 'HEAD' || !options.jQuery) {
    return options.callback(null, response, options.release, self);
  }
...
Crawler.prototype._injected = function(errors, response, options, $){
  ...
  response.$ = $;
  options.callback(errors, response, options.release, self);
}
...
Crawler.prototype._doEncoding = function(options,response) {
  ...
  if (options.forceUTF8) {
    let charset = options.incomingEncoding || self._parseCharset(response);
...
Crawler.prototype._parseCharset = function(res){
  ...
  let body = res.body instanceof Buffer?res.body.toString():res.body;
